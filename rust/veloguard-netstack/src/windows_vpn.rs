//! Windows VPN packet processor using SolidTCP stack
//!
//! This module provides the integration between Windows TUN device (wintun)
...
pub struct WindowsVpnProcessor {
    /// The SolidTCP stack
...
pub struct WindowsVpnTrafficStats {
...
impl WindowsVpnProcessor {
    /// Create a new Windows VPN processor
    pub fn new(proxy_port: u16, tun_tx: mpsc::Sender<BytesMut>) -> Self {
        info!("Creating WindowsVpnProcessor with proxy_port={}", proxy_port);
        ...
        info!("SolidTCP stack started for Windows VPN");
    ...
    pub fn get_traffic_stats(&self) -> WindowsVpnTrafficStats {
        let stats = self.stack.stats();
        let snapshot = stats.snapshot();
        WindowsVpnTrafficStats {
    ...
    pub fn stop(&self) {
        ...
        info!("Windows VPN processor stopped, processed {} packets total", total);
    }

    /// Reset the Fake-IP pool
    pub fn reset_fake_ip_pool(&self) {
        self.stack.fake_ip_pool().clear();
        info!("Fake-IP pool reset via WindowsVpnProcessor");
    }
    ...
    pub fn reset(&self) {
        ...
        info!("WindowsVpnProcessor fully reset");
    }

    /// Get the TUN sender channel
    pub fn tun_sender(&self) -> mpsc::Sender<BytesMut> {
        self.tun_tx.clone()
    }
}
