use super::crypto::{
    aead_decrypt, aead_encrypt, dh, hash, kdf1, kdf2, kdf3, mac, timestamp, CONSTRUCTION,
    IDENTIFIER, LABEL_MAC1,
};
...
impl HandshakeState {
    ...
    pub fn consume_response(
        ...
        if response.receiver_index != self.local_index {
            return Err(WireGuardError::Handshake(
                "Invalid receiver index".to_string(),
            ));
        }
        ...
        if !decrypted.is_empty() {
            return Err(WireGuardError::Handshake(
                "Invalid response payload".to_string(),
            ));
        }
    ...
    pub fn process_initiation(
        ...
        if decrypted_static.len() != 32 {
            return Err(WireGuardError::Handshake(
                "Invalid static key length".to_string(),
            ));
        }
    ...
    pub fn create_response(
        &mut self,
    ) -> Result<(HandshakeResponse, TransportKeys), WireGuardError> {
...
impl HandshakeInitiation {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, WireGuardError> {
        if data.len() < 148 {
            return Err(WireGuardError::Handshake(
                "Initiation message too short".to_string(),
            ));
        }
...
impl HandshakeResponse {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, WireGuardError> {
        if data.len() < 92 {
            return Err(WireGuardError::Handshake(
                "Response message too short".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_handshake_roundtrip() {
        ...
        let mut initiator =
            HandshakeState::new_initiator(initiator_priv, initiator_pub, responder_pub, None);
        ...
        let mut responder = HandshakeState::new_responder(responder_priv, responder_pub, None);
    ...
    fn test_handshake_with_psk() {
        ...
        let mut initiator =
            HandshakeState::new_initiator(initiator_priv, initiator_pub, responder_pub, Some(psk));

        let init_msg = initiator.create_initiation().unwrap();

        let mut responder = HandshakeState::new_responder(responder_priv, responder_pub, Some(psk));
