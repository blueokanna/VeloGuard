...
fn parse_ipv4(data: &[u8]) -> Result<ParsedPacket> {
    ...
    let src = Ipv4Addr::from(pkt.src_addr());
    let dst = Ipv4Addr::from(pkt.dst_addr());
...
fn parse_ipv6(data: &[u8]) -> Result<ParsedPacket> {
    ...
    let src = Ipv6Addr::from(pkt.src_addr());
    let dst = Ipv6Addr::from(pkt.dst_addr());
...
pub fn build_ipv4_tcp(
    ...
    // IP Identification - must be unique per packet
    let ip_id = IP_ID.fetch_add(1, Ordering::Relaxed);
    pkt[4..6].copy_from_slice(&ip_id.to_be_bytes());
    
    pkt[6..8].copy_from_slice(&0x4000u16.to_be_bytes()); // DF flag, no fragmentation
    pkt[8] = 64; // TTL
    ...
    // IP checksum (with checksum field = 0)
    ...
    pkt[tcp_start+12] = ((tcp_hdr_len / 4) as u8) << 4;  // Data offset
    pkt[tcp_start+13] = flags.to_byte();
    pkt[tcp_start+14..tcp_start+16].copy_from_slice(&window.to_be_bytes());
    // Checksum at offset 16-17 (calculated below)
    // Urgent pointer at offset 18-19 (already zero)

    // MSS option (only for SYN packets)
    if flags.syn {
        if let Some(mss_val) = mss {
            pkt[tcp_start+20] = 2;  // MSS option kind
            pkt[tcp_start+21] = 4;  // MSS option length
    ...
    // TCP checksum (calculated over pseudo-header + TCP header + payload)
    // Checksum field must be 0 during calculation (already is)
