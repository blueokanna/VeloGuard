...
impl SelectorOutbound {
    pub fn new(config: OutboundConfig, registry: ProxyRegistry) -> Result<Self> {
        ...
        let outbounds: Vec<String> = if let Some(outbounds_value) = config.options.get("outbounds")
        {
            ...
            if let Some(arr) = outbounds_value.as_sequence() {
                let result: Vec<String> = arr
                    ...
                    .collect();
                tracing::debug!(
                    "Selector '{}' parsed {} outbounds from sequence",
                    config.tag,
                    result.len()
                );
                result
                ...
                tracing::warn!(
                    "Selector '{}' outbounds_value is neither sequence nor string",
                    config.tag
                );
                Vec::new()
            }
            ...
            tracing::warn!(
                "Selector '{}' has no 'outbounds' in options. Available keys: {:?}",
                config.tag,
        ...
        let default_selected = outbounds
            .first()
            .cloned()
            .unwrap_or_else(|| "DIRECT".to_string());

        tracing::info!(
            "Selector '{}' created with {} outbounds: {:?}, default: {}",
            config.tag,
            outbounds.len(),
            outbounds,
            default_selected
    ...
    pub fn set_selected(&self, tag: &str) -> Result<()> {
        if self.outbounds.contains(&tag.to_string()) || tag == "DIRECT" || tag == "REJECT" {
            ...
            Err(Error::config(format!(
                "Outbound '{}' not in selector group '{}'",
                tag, self.config.tag
            )))
        }
    }
    ...
    async fn resolve_proxy(&self, max_depth: usize) -> Result<Arc<dyn OutboundProxy>> {
        ...
        if let Some(proxy) = self.find_proxy(&selected).await {
            ...
            Err(Error::config(format!(
                "Selected outbound '{}' not found in registry",
                selected
            )))
        }
    }
...
impl OutboundProxy for SelectorOutbound {
    ...
    async fn relay_udp_packet(&self, target: &TargetAddr, data: &[u8]) -> Result<Vec<u8>> {
        ...
        tracing::debug!(
            "Selector '{}' relaying UDP to '{}' for target {}",
            self.config.tag,
            proxy.tag(),
            target
        );
