    ...
    async fn relay_tcp_with_connection(
        ...
        let client_to_remote = async {
            ...
            loop {
                let n = match ri.read(&mut buf).await {
                    Ok(0) => break,
                    Ok(n) => n,
                    Err(e) if is_trojan_connection_closed(&e) => {
                        tracing::debug!("Trojan: client closed connection");
                        break;
                    }
                ...
                if let Err(e) = wo.write_all(&buf[..n]).await {
                    if is_trojan_connection_closed(&e) {
                        tracing::debug!("Trojan: server closed connection");
                        break;
                    }
                    return Err(Error::network(format!("Failed to write to Trojan: {}", e)));
                }
        ...
        let remote_to_client = async {
            ...
            loop {
                let n = match ro.read(&mut buf).await {
                    ...
                    Err(e) => return Err(Error::network(format!("Failed to read from Trojan: {}", e))),
                };
        ...
        match (upload_result, download_result) {
            ...
            (Ok(_), Err(e)) | (Err(e), Ok(_)) => {
                tracing::debug!("Trojan relay partial error: {}", e);
                Ok(())
            }
            (Err(e1), Err(e2)) => {
                tracing::debug!("Trojan relay errors: {} / {}", e1, e2);
                Ok(())
            }
        }
    }
...
/// Check if an error indicates a normal connection close for Trojan
fn is_trojan_connection_closed(e: &std::io::Error) -> bool {
    matches!(
        e.kind(),
