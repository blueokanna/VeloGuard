//! VeloGuard QUIC server implementation
...
use crate::config::{CongestionControl, ServerConfig};
use crate::crypto::CryptoContext;
use crate::error::{Result, QuicError};
use crate::protocol::{Command, Request, Response, ResponseStatus, UdpHeader};
use crate::stream::{QuicStream, StreamType};

/// VeloGuard QUIC server
...
impl QuicServer {
    ...
    pub async fn accept(&self) -> Result<Option<Arc<ServerConnection>>> {
        let endpoint = self
            .endpoint
            .as_ref()
            .ok_or(QuicError::ServerNotReady)?;
...
impl ServerConnection {
    ...
    pub async fn handle(&self) -> Result<()> {
        loop {
            match self.inner.accept_bi().await {
                Ok((send, recv)) => {
                    let _crypto = CryptoContext::new(
                        self.crypto.cipher_kind(),
                        "", // Will be set from request
                    );
    ...
    async fn handle_stream(
        ...
        match request.command {
            Command::Connect => {
                Self::handle_tcp_connect(stream, request, fallback).await
            }
    ...
    async fn handle_tcp_connect(
        ...
        // If there's early data, send it first
    ...
    /// Relay TCP data between QUIC stream and TCP connection
    async fn relay_tcp(
        mut quic_send: crate::stream::QuicSendStream,
        mut quic_recv: crate::stream::QuicRecvStream,
